...
const getFollowers = async () => {
  if (Date.now() > lastRefresh + refreshTimeout) {
    try {
      const response = await axios.get(`${apiBaseUrl}/users/follows?to_id=${config.twitchUserId}&first=100`, twitchHeaders);
      ...
      while (localCount < totalCount) {
        ...
        const nextResponse = await axios.get(`${apiBaseUrl}/users/follows?first=100&after=${lastCursor}&to_id=${config.twitchUserId}`, twitchHeaders);
...
router.get('/', async (req, res) => {
  const follows = await getFollowers();
  res.json(follows);
});

router.get('/total', async (req, res) => {
  const follows = await getFollowers();
  res.json(follows.data.length);
});

router.get('/goal', async (req, res) => {
  const follows = await getFollowers();
  const goalArray = [250, 300, 400, 500, 750, 1000, 2000, 5000].filter(
    (c) => follows.data.length < c
  );

  res.json({ total: follows.data.length, goal: goalArray[0] });
});
